        ...
        try:
            ...
            try:
                ...
                if references:
                    # 使用清理后的文本 + 参考文献
                    clean_text_for_display = extractor.clean_text_for_tts(orig_text)
                    # 如果清理后的文本为空，使用原文的部分内容
                    if not clean_text_for_display.strip():
                        clean_text_for_display = orig_text[:200] + "..." if len(orig_text) > 200 else orig_text
                    
                    # 验证参考文献内容
                    if references.strip():
                        logging.info("TTS: sending references with audio")
                        result.chain = [Plain(text=clean_text_for_display + references), record]
                    else:
                        logging.warning("TTS: references is empty, sending without references")
                        result.chain = [Plain(text=clean_text_for_display), record]
                else:
