...
class TTSEmotionRouter(Star):
    ...
    async def on_llm_response(self, event: AstrMessageEvent, response: LLMResponse):
        """
        LLM回复后，尝试进行TTS语音合成。兼容重试插件：只在有内容时处理。
        """
        # 1. 只在completion_text非空时处理
        text = getattr(response, "completion_text", None)
        if not text or not text.strip():
            return
        # 2. 归一化、情绪标签处理（始终strip情绪头）
        t0 = self._normalize_text(text)
        cleaned, label = self._strip_emo_head_many(t0)
        if cleaned:
            text = cleaned
            response.completion_text = cleaned
            try:
                ...
                pass
        # 3. 同步去除result_chain中所有Plain的情绪头
        rc = getattr(response, "result_chain", None)
        if rc and hasattr(rc, "chain"):
            ...
            for comp in rc.chain:
                if isinstance(comp, Plain) and getattr(comp, "text", None):
                    ...
                    new_chain.append(Plain(text=t) if t else comp)
                else:
                    new_chain.append(comp)
            rc.chain = new_chain
        # 3. 语音合成概率、冷却、字数等判定（复用原有判定）
        sid = self._sess_id(event)
        ...
        if random.random() > self.prob:
            return
        # 4. 情绪选择
        emotion = label if label in EMOTIONS else self.heuristic_cls.classify(text, context=None)
        ...
        if rc and hasattr(rc, "chain"):
            rc.chain.clear()
            # 只要TTS合成成功（audio_path存在且有效），就输出Record，否则只输出Plain
            if 'audio_path' in locals() and audio_path:
                ...
                record_obj = Record(file=audio_file_path)
                if self.allow_mixed:
                    rc.chain.append(Plain(text=text))
                    rc.chain.append(record_obj)
                else:
                    rc.chain.append(record_obj)
            else:
                rc.chain.append(Plain(text=text))
        ...
        # 9. 缓存Plain文本，供兜底入库
        try:
            setattr(response, "_tts_plain_text", text)
        except Exception:
            pass
        st.last_tts_content = text
        st.last_tts_time = time.time()
...
🎭 音色配置:"""
    ...
    async def on_decorating_result(self, event: AstrMessageEvent):
        # 只保留兜底日志和continue_event，主TTS逻辑已迁移到on_llm_response
        try:
            event.continue_event()
        except Exception:
            pass
